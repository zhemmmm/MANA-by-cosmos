...
// ─── Toggle Pin ───────────────────────────────────────────────────────────────
const POST_CAPTION_COLLAPSE_LIMIT = 180;
...
async function renderClusterDetail() {
  ...
  document.getElementById("clusterHero").innerHTML = `
    ...
    <div class="cluster-metrics">
      <div class="mini-card kpi-blue"><div class="mini-card-label">Total posts in cluster</div><div class="mini-card-value">${formatNumber(baseClusterPosts.length)}</div><div class="mini-card-meta">Posts shown in this cluster</div></div>
  ...
  const filteredPosts = baseClusterPosts
    .filter(p => {
